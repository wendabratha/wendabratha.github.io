var config = {
  geojson: "http://geoportal.kukarkab.go.id:8080/geoserver/t_bidang_pendapatan_daerah/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=t_bidang_pendapatan_daerah%3Aobjekpajak&maxFeatures=10000&outputFormat=application%2Fjson",
  title: "Sistem Informasi Objek Pajak Daerah",
  layerName: "Objek Pajak Daerah",
  hoverProperty: "kategoripajak_id",
  sortProperty: "kategoripajak_id",
  sortOrder: "desc"
};
...
function drawCharts() {
  // Jenis Objek
  $(function() {
    var result = alasql("SELECT nama_kategori AS label, COUNT(*) AS total FROM ? GROUP BY nama_kategori", [features]);
    var columns = $.map(result, function(jenisObjek) {
      return [[jenisObjek.label, jenisObjek.total]];
    });
    var chart = c3.generate({
        bindto: "#jenisObjek-chart",
        data: {
          type: "bar",
          columns: columns
        }
  ...
  // Desa
  $(function() {
    var result = alasql("SELECT desa AS label, COUNT(*) AS total FROM ? GROUP BY desa", [features]);
    var columns = $.map(result, function(desa) {
      return [[desa.label, desa.total]];
    });
    var chart = c3.generate({
        bindto: "#desa-chart",
        data: {
  ...
  // Kecamatan
  $(function() {
    var result = alasql("SELECT kecamatan AS label, COUNT(*) AS total FROM ? GROUP BY kecamatan ORDER BY label ASC", [features]);
    var chart = c3.generate({
        bindto: "#kecamatan-chart",
        size: {
          height: 500
        },
        data: {
...
var mapboxOSM = L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
  maxZoom: 19,
  attribution: 'Geoportal Kutai Kartanegara &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors</a>'
});
